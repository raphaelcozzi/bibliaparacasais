...
class cadastro extends home
{
	...
	function insere()
	{
	   ...
	   $sql = "INSERT INTO admin_usuarios (nome, email, senha, estado, data_cadastro, status, cidade, alert_daily, eventos_lote, lancamentos_lote, endereco, pais) 
	   ...
	   $id_usuario = $db->get_last_insert_id("usuarios","id");
           ...
           $sql = "UPDATE admin_usuarios SET usuario_master = ".$id_usuario." WHERE id = ".$id_usuario." LIMIT 1 "; 
	   $db->query($sql,__LINE__,__FILE__);
	   $db->next_record();
	...
	function valid()
	{
	   ...
	   $sql = "SELECT id as id_usuario FROM admin_usuarios WHERE email = '".$email."' LIMIT 1 ";
	   $db->query($sql,__LINE__,__FILE__);
	   $db->next_record();
	   ...
	   if($db->num_rows() > 0)
	   {
		   ...
		   if($db->num_rows() > 0)
		   {
			   ...
			   $sql = "UPDATE admin_usuarios SET status = 1 WHERE id = ".$id_usuario." LIMIT 1 ";
			   $db->query($sql,__LINE__,__FILE__);
			   $db->next_record();
